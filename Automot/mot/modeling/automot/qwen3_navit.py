...
from modeling.qwen3.modeling_qwen3 import (
    Qwen3Attention, 
    Qwen3MLP, 
...
from modeling.qwen3.configuration_qwen3 import Qwen3Config as _Qwen3Config
...
class Qwen3Config(_Qwen3Config):
    ...
    >>> # Initializing a model from the Qwen3-4B-Think style configuration
    ...
    def __init__(
        ...
        freeze_und=False,
        # MoT-specific parameters for configurable attention heads
        mot_num_attention_heads=16,
        mot_num_key_value_heads=4,
        # MoT-specific MLP size
        mot_intermediate_size=4864,
        **kwargs,
    ):
        super().__init__(
        ...
        # Set default MoT head counts if not specified (half of regular attention heads)
        ...
        # Set default MoT intermediate size if not specified (same as regular intermediate size)
        self.mot_intermediate_size = mot_intermediate_size if mot_intermediate_size is not None else intermediate_size
...
class PackedAttentionMoT(Qwen3Attention):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        # MoT-specific head configuration
        ...
        if self.config.qk_norm:
            self.q_norm = Qwen3RMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.k_norm = Qwen3RMSNorm(self.head_dim, eps=config.rms_norm_eps)
            self.q_norm_moe_gen = Qwen3RMSNorm(self.mot_head_dim, eps=config.rms_norm_eps)
            self.k_norm_moe_gen = Qwen3RMSNorm(self.mot_head_dim, eps=config.rms_norm_eps)
        else:
            self.q_norm = nn.Identity()
            self.k_norm = nn.Identity()
        ...
        ### Oscar: Use MoT head numbers and dimension ####
        self.q_proj_moe_gen = nn.Linear(self.hidden_size, self.mot_num_heads * self.mot_head_dim, bias=self.config.attention_bias)
        self.k_proj_moe_gen = nn.Linear(self.hidden_size, self.mot_num_key_value_heads * self.mot_head_dim, bias=self.config.attention_bias)
        self.v_proj_moe_gen = nn.Linear(self.hidden_size, self.mot_num_key_value_heads * self.mot_head_dim, bias=self.config.attention_bias)
        self.o_proj_moe_gen = nn.Linear(self.mot_num_heads * self.mot_head_dim, self.hidden_size, bias=self.config.attention_bias)
    ...
    def forward_train(
        ...
        max_heads_q = max(self.num_heads, self.mot_num_heads)
        max_heads_kv = max(self.num_key_value_heads, self.mot_num_key_value_heads)
        max_head_dim = max(self.head_dim, self.mot_head_dim)
        
        ### Oscar: Padding with zero as it will not affect the attention calculation, no need mask operation here ###
        ...
        if len(packed_und_token_indexes) > 0:
            und_q = self.q_proj(packed_sequence_und).view(-1, self.num_heads, self.head_dim)
            und_k = self.k_proj(packed_sequence_und).view(-1, self.num_key_value_heads, self.head_dim)
            und_v = self.v_proj(packed_sequence_und).view(-1, self.num_key_value_heads, self.head_dim)
        ...
        if len(packed_gen_token_indexes) > 0:
            gen_q = self.q_proj_moe_gen(packed_sequence_gen).view(-1, self.mot_num_heads, self.mot_head_dim)
            gen_k = self.k_proj_moe_gen(packed_sequence_gen).view(-1, self.mot_num_key_value_heads, self.mot_head_dim)
            gen_v = self.v_proj_moe_gen(packed_sequence_gen).view(-1, self.mot_num_key_value_heads, self.mot_head_dim)
        ...
        if len(packed_gen_token_indexes) > 0:
            packed_query_states_[packed_gen_token_indexes, :self.mot_num_heads, :self.mot_head_dim] = self.q_norm_moe_gen(
                packed_query_states[packed_gen_token_indexes, :self.mot_num_heads, :self.mot_head_dim]
            )
            packed_key_states_[packed_gen_token_indexes, :self.mot_num_key_value_heads, :self.mot_head_dim] = self.k_norm_moe_gen(
        ...
        if isinstance(attention_mask, List):
            ...
            max_heads_total = max(self.num_heads, self.mot_num_heads)
            ...
            if len(packed_und_token_indexes) > 0:
                ...
                packed_key_states_expanded[packed_und_token_indexes] = und_key_expanded.reshape(-1, self.num_heads, self.head_dim)
                
                und_value_expanded = packed_value_states[packed_und_token_indexes][:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1)
                packed_value_states_expanded[packed_und_token_indexes] = und_value_expanded.reshape(-1, self.num_heads, self.head_dim)
        ...
        if len(packed_und_token_indexes) > 0:
            und_output = packed_attn_output[packed_und_token_indexes].reshape(-1, self.num_heads * self.head_dim)
        ...
        if len(packed_gen_token_indexes) > 0:
            ...
            packed_attn_output_[packed_gen_token_indexes] = self.o_proj_moe_gen(gen_output)

        return packed_attn_output_

    ### Oscar: To be tested !!! Same as Qwen3MoTDecoderLayer Inference ###
    def forward_inference(
        self,
        packed_query_sequence: torch.Tensor,
        query_lens: torch.Tensor,
        ...
        if mode == 'und':
            ...
            # Use maximum dimensions for mixed head configurations
            max_q_dim = max(self.num_heads * self.head_dim, self.mot_num_heads * self.mot_head_dim)
            max_kv_dim = max(self.num_key_value_heads * self.head_dim, self.mot_num_key_value_heads * self.mot_head_dim)
            
            packed_query_states = packed_query_sequence.new_zeros((packed_query_sequence.shape[0], max_q_dim))
            packed_key_states = packed_query_sequence.new_zeros((packed_query_sequence.shape[0], max_kv_dim))
            packed_value_states = packed_query_sequence.new_zeros((packed_query_sequence.shape[0], max_kv_dim))

            packed_text_query_sequence = packed_query_sequence[packed_text_indexes]
            packed_vae_query_sequence = packed_query_sequence[packed_vae_token_indexes]

            # Apply projections and store in appropriate dimensions
            if len(packed_text_indexes) > 0:
                packed_query_states[packed_text_indexes, :self.num_heads * self.head_dim] = self.q_proj(packed_text_query_sequence)
                packed_key_states[packed_text_indexes, :self.num_key_value_heads * self.head_dim] = self.k_proj(packed_text_query_sequence)
                packed_value_states[packed_text_indexes, :self.num_key_value_heads * self.head_dim] = self.v_proj(packed_text_query_sequence)

            if len(packed_vae_token_indexes) > 0:
                packed_query_states[packed_vae_token_indexes, :self.mot_num_heads * self.mot_head_dim] = self.q_proj_moe_gen(packed_vae_query_sequence)
                packed_key_states[packed_vae_token_indexes, :self.mot_num_key_value_heads * self.mot_head_dim] = self.k_proj_moe_gen(packed_vae_query_sequence)
                packed_value_states[packed_vae_token_indexes, :self.mot_num_key_value_heads * self.mot_head_dim] = self.v_proj_moe_gen(packed_vae_query_sequence)

            # Reshape to head dimensions with maximum dimensions
            max_heads_q = max(self.num_heads, self.mot_num_heads)
            max_heads_kv = max(self.num_key_value_heads, self.mot_num_key_value_heads)
            max_head_dim = max(self.head_dim, self.mot_head_dim)
            
            query_reshaped = packed_query_sequence.new_zeros((packed_query_sequence.shape[0], max_heads_q, max_head_dim))
            key_reshaped = packed_query_sequence.new_zeros((packed_query_sequence.shape[0], max_heads_kv, max_head_dim))
            value_reshaped = packed_query_sequence.new_zeros((packed_query_sequence.shape[0], max_heads_kv, max_head_dim))
            
            # Reshape text tokens
            if len(packed_text_indexes) > 0:
                query_reshaped[packed_text_indexes] = packed_query_states[packed_text_indexes, :self.num_heads * self.head_dim].view(-1, self.num_heads, self.head_dim)
                key_reshaped[packed_text_indexes] = packed_key_states[packed_text_indexes, :self.num_key_value_heads * self.head_dim].view(-1, self.num_key_value_heads, self.head_dim)
                value_reshaped[packed_text_indexes] = packed_value_states[packed_text_indexes, :self.num_key_value_heads * self.head_dim].view(-1, self.num_key_value_heads, self.head_dim)
            
            # Reshape vae tokens
            if len(packed_vae_token_indexes) > 0:
                query_reshaped[packed_vae_token_indexes, :self.mot_num_heads, :self.mot_head_dim] = packed_query_states[packed_vae_token_indexes, :self.mot_num_heads * self.mot_head_dim].view(-1, self.mot_num_heads, self.mot_head_dim)
                key_reshaped[packed_vae_token_indexes, :self.mot_num_key_value_heads, :self.mot_head_dim] = packed_key_states[packed_vae_token_indexes, :self.mot_num_key_value_heads * self.mot_head_dim].view(-1, self.mot_num_key_value_heads, self.mot_head_dim)
                value_reshaped[packed_vae_token_indexes, :self.mot_num_key_value_heads, :self.mot_head_dim] = packed_value_states[packed_vae_token_indexes, :self.mot_num_key_value_heads * self.mot_head_dim].view(-1, self.mot_num_key_value_heads, self.mot_head_dim)
            
            packed_query_states = query_reshaped
            packed_key_states = key_reshaped
            packed_value_states = value_reshaped
            ...
            if len(packed_vae_token_indexes) > 0:
                packed_query_states[packed_vae_token_indexes, :self.mot_num_heads, :self.mot_head_dim] = self.q_norm_moe_gen(packed_query_states[packed_vae_token_indexes, :self.mot_num_heads, :self.mot_head_dim])
                packed_key_states[packed_vae_token_indexes, :self.mot_num_key_value_heads, :self.mot_head_dim] = self.k_norm_moe_gen(packed_key_states[packed_vae_token_indexes, :self.mot_num_key_value_heads, :self.mot_head_dim])
        ...
        if mode == 'und':
            ...
            packed_attn_output_proj = packed_attn_output.new_zeros(packed_attn_output.shape[0], self.hidden_size)
            
            if len(packed_text_indexes) > 0:
                text_output = packed_attn_output[packed_text_indexes].reshape(-1, self.hidden_size)
                packed_attn_output_proj[packed_text_indexes] = self.o_proj(text_output)
            
            if len(packed_vae_token_indexes) > 0:
                # Extract only the MoT dimensions for VAE tokens
                vae_output = packed_attn_output[packed_vae_token_indexes, :self.mot_num_heads * self.mot_head_dim].reshape(-1, self.mot_num_heads * self.mot_head_dim)
                packed_attn_output_proj[packed_vae_token_indexes] = self.o_proj_moe_gen(vae_output)
...
class Qwen3MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        ### Oscar: Reduce the MLP size!!! Create a modified config for MoT MLP with custom intermediate_size ###
        mot_mlp_config = type(config)(**config.__dict__)
        mot_mlp_config.intermediate_size = config.mot_intermediate_size # e.g.: from 9728 -> 4864
        self.mlp_moe_gen = Qwen3MLP(mot_mlp_config)
        self.input_layernorm = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.input_layernorm_moe_gen = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm_moe_gen = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config):
        ...
        if self.use_moe:
            self.norm_moe_gen = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
