...
class SftJSONLIterableDatasetSenna(DistributedIterableDataset):
    def __init__(
    ...
    def change_format(self, data, num_images, num_fronts, num_lidars):
        elements = []
        ## --jh-- change the format to include front
        for conversation in data['conversations']:
            if conversation['from'] == 'human':
                ...
                inf_type = data.get('inference_type') or conversation.get('inference_type')
            ...
            for row_idx, (data, image_dir) in enumerate(data_paths_per_worker_, start=row_start_id):
                ...
                if raw_fronts:
                    for raw_front in raw_fronts:
                        h, w = raw_front.size[1], raw_front.size[0]
                        raw_front = self.resize_image(raw_front, width=512, height=256) 
                        front_dic = self.processor(
                        text='',
                        images=raw_front, 
                        return_tensors="pt"
                        )
