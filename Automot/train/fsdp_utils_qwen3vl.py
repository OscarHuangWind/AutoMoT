...
from mot.modeling.automot.modeling_utils import MLPconnector, TimestepEmbedder, PositionEmbedding
from mot.modeling.automot.qwen3vl_navit import (
    Qwen3VLDecoderLayer,
    Qwen3VLMoTDecoderLayer,
)
...
class FSDPCheckpoint:
    ...
    def fsdp_save_ckpt(
        ...
        # Save configs and tokenizer (only rank 0)
        if dist.get_rank() == 0:
            ...
            with open(os.path.join(save_path, "vision_config.json"), "w") as f:
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        if resume_from is not None and os.path.exists(resume_from):
            ...
            # Fixed sinusoidal position embeddings can be regenerated for new resolutions.
