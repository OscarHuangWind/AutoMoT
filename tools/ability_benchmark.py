...
def main(args):
    ...
    if not carla_path:
        # Auto-detect: try parent of project root or ~/carla
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir)
        for candidate in [os.path.join(os.path.dirname(project_root), 'carla'), os.path.expanduser('~/carla')]:
            if os.path.isfile(os.path.join(candidate, 'CarlaUE4.sh')):
                carla_path = candidate
                break
        if not carla_path:
            raise EnvironmentError("CARLA_ROOT is not set and could not be auto-detected.")
        print(f"Auto-detected CARLA_ROOT={carla_path}")
    ...
    for route in sorted_routes:
        ...
        if scenario_name in Ability["Traffic_Signs"]:
            ...
            if not wp.is_junction:
                print(f"WARNING: Route {route_id} ({scenario_name}) does not contain any junction-waypoint, skipping Traffic_Signs check.")
                continue
