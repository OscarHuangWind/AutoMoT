...
Usage:
    python tools/upload_to_hf.py --token YOUR_HF_WRITE_TOKEN --username YOUR_HF_USERNAME
...
Use --delete-old to remove old automot/ and transfuser/ folders from HF repo.
"""
...
BEV_ENCODER   = REPO_ROOT / "Automot" / "checkpoints" / "bev_encoder"
...
def delete_hf_folder(api: HfApi, repo_id: str, folder: str):
...
def main():
    ...
    parser.add_argument("--delete-old", action="store_true", default=False,
                        help="Delete old automot/ and transfuser/ folders from HF repo")
    ...
    # Optionally clean up old separate checkpoints
    if args.delete_old:
        print("\n[Deleting old automot/ and transfuser/ from HF repo...]")
        delete_hf_folder(api, repo_id, "automot")
        delete_hf_folder(api, repo_id, "transfuser")
